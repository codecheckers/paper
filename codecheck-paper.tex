...
%\textbf{Conclusions:}
 ...
 To help improve this situation, we propose a system by which the computational workflows underlying research articles are checked.
...
or the CODECHECKER concludes the workflow is not reproducible.
...
an open archive unless restrictions (data size, license or 
sensitivity) apply.
...
Although a badge simplifies the CODECHECK into a binary value and risks introducing confusion regarding the extent of the check, especially across disciplines and methods, a badge can provide recognition value and acknowledges the completed CODECHECK.
The badge and the actual check are incentives for undertaking the effort needed to provide a reproducible workflow.

\textbf{Step 6:} The PUBLISHER can, depending on the timing, provide the
certificate to peer-reviewers or editors or publish it
...
the activity in scholarly profiles, such as ORCID
...
on GitHub or Open Science Framework (OSF), contains the CODECHECK
...
increase overall reproducibility, and it may prohibit authors from
sharing their best efforts, which is always more desirable than
nothing shared. Therefore, we recommend sharing the latest
draft of the certificate only with the authors, even if that
means that volunteer efforts may go unnoticed if no report is
published.  The most appropriate way forwards seems to be to give
private feedback for failures, public acknowledgment and
encouragement for successes, and recognise codecheckers as reviewers.
