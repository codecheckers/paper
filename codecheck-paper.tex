...
\maketitle
\begin{abstract}
  The scientific paper falls short of effectively communicating computational research.
  To break out of the stalemate between multiple lacks of incentives, guidance, benefits, recognitions, and reproducibility, we propose to introduce execution of workflows underlying research articles as part of peer review.
  The CODECHECK system relies on five principles and can be integrated into review and publications processes in multiple ways, which we describe along multiple dimensions (importance, who, openness, when).
  We demonstrate CODECHECK with 25 reproductions of diverse scientific publications in collaboration with academic publishers and conferences.
  These CODECHECKs show that asking for reproducible workflows in a collaborative review can effectively improve executability.
  CODECHECK relies on free infrastructure and open tools.
  While it has clear limitations, it can be a building block in the open-science and publishing ecosystems to improve the reproducibility, appreciation, and potentially quality of non-textual research artefacts.
\end{abstract}
...
\textbf{Aren't CODECHECK requirements not demanding enough?}
...
reproductions very \textbf{educational}. It's a different role and as such could
